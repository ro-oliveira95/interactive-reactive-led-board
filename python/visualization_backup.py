...
def visualize_energy(y):
    ...
    p[0, :] = gaussian_filter1d(p[0, :], sigma=4.0) / 2##
    p[1, :] = gaussian_filter1d(p[1, :], sigma=4.0) / 2##
    p[2, :] = gaussian_filter1d(p[2, :], sigma=4.0) / 2##
...
def map1dto2d_plot(pixels):
    ...
    for i in range(len(mapped_pixels[0])):
        # if int(i/width)%2!=0:
        #     columIndex = invertColumnIndex(i%(width))
        # else:
        #     columIndex = i%(width)
    ...
    # mapped_pixels = [np.zeros(config.N_PIXELS), np.zeros(config.N_PIXELS), np.zeros(config.N_PIXELS)]
...
def testing ():
    rgb = [np.zeros(config.N_PIXELS), np.zeros(config.N_PIXELS), np.zeros(config.N_PIXELS)]
...
def microphone_update(audio_samples):
    ...
    if vol < config.MIN_VOLUME_THRESHOLD:
        ...
        """ ---------------
            MAPPING 1d to 2d
            --------------- """
        output = map1dto2d(output)
        # output = map1dto2d_plot(output)
        # output = sin_static()
        ...
        if config.USE_GUI:
            ...
            # r_curve.setData(y=np.floor(led.pixels[0] / 32))
            # g_curve.setData(y=np.floor(led.pixels[1] / 32))
            # b_curve.setData(y=np.floor(led.pixels[2] / 32))
...
if __name__ == '__main__':
    if config.USE_GUI:
        ...
        r_brush = pg.mkBrush(255, 30, 30, 200)
        g_brush = pg.mkBrush(30, 255, 30, 200)
        b_brush = pg.mkBrush(30, 30, 255, 200)

        # Color channel curves
        r_curve = pg.PlotDataItem(pen=r_pen)
        g_curve = pg.PlotDataItem(pen=g_pen)
        b_curve = pg.PlotDataItem(pen=b_pen)
        # Define x data
